...
class NavbarController extends Controller
{
    ...
    public function explore_lawyer(Request $request)
    {
        ...
        $query = User::where('is_active', true)
                     ->where('type', UserTypeEnum::lawyer);

        if ($request->has('practices')) {
            $practiceIds = $request->input('practices');
            if (!empty($practiceIds)) {
                $query->whereHas('practices', function ($query) use ($practiceIds) {
                    $query->whereIn('practices.id', $practiceIds)
                          ->groupBy('users.id')
                          ->havingRaw('COUNT(DISTINCT practices.id) = ?', [count($practiceIds)]);
                });
            }
        }
        ...
        if ($request->ajax()) {
            $lawyers = $lawyers->map(function ($lawyer) {
                return [
                    'encoded_id' => base64_encode($lawyer->id),
                    'name' => $lawyer->name,
        ...
        return view('pages.navbar.explore_lawyer', compact('practices', 'languages', 'lawyers'));
    }
